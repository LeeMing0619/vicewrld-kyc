...
export default memo(function UserDialog() { console.log("asdfasdf")
  ...
  const [changeIsPremium] = useChangeIsPremiumMutation()
  ...
  const buttons = useMemo(() => {
    if (user?.relationshipStatus === RelationshipStatus.FollowedBy)
      return (
        <>
          <button
    ...
    else if (user?.relationshipStatus === RelationshipStatus.Following)
      return (
        <button
          className={buttonClass(false)}
          onClick={() =>
    ...
    else if (user?.relationshipStatus === RelationshipStatus.MutualFollow)
      return (
        <Link
          to={`/dm/@${user?.username}`}
          onClick={() => close()}
        ...
        {!currentUser || user?.id !== currentUser.id ? (
          <>
            <div className="px-5 dark:border-gray-775 border-t h-14 flex items-center space-x-10">
              ...
              >
                <div className="transform translate-y-0.5">Mutual Rooms</div>
              </button>
              <button
            ...
            <div className="rounded-b-lg dark:bg-gray-750 p-2 max-h-[15rem] min-h-[15rem] h-full scrollbar-custom">
              {currentTab === tab.MutualServers &&
                (mutualServers.length > 0 ? (
                  ...
                  <EndReached className="h-36">No mutual rooms</EndReached>
                ))}
