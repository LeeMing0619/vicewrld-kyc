...
export default function FriendsPage() {
  const { friends, following, followedBy, blocking } = useUserRelationships()
  const onlineFriends = friends.filter(f => f.isOnline)
  const friendRequests = following.concat(followedBy)
  ...
  return (
    <Page header={<FriendsHeader pendingCount={followedBy.length} />}>
      <Helmet>
          ...
          <>
            ...
            {friendRequests.map(user => (
              <FriendRequestListItem
                user={user}
                key={`${
                  user.relationshipStatus === RelationshipStatus.Following
                    ? 'following'
                    : 'followedBy'
                }-${user.id}`}
              />
