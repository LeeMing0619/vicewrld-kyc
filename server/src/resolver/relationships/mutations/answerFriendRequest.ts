...
export async function answerFriendRequest(
  ...
  if (
    !(
      myData.status === RelationshipStatus.FollowedBy &&
      theirData.status === RelationshipStatus.Following
    )
  )
    throw new Error('Invalid relationship status')

  if (accept) {
    myData.status = RelationshipStatus.MutualFollow
    theirData.status = RelationshipStatus.MutualFollow
  } else {
